...
uint256 hashGenesisBlock("0xb74cefd2b3307ae268eb250be7ec60e4b910408e86b033ed6beaef3093fe7bdb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Reserved: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Reserved Signed Message:\n";
...
{
    // Reserved: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Reserved: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Reserved: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Reserved: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Reserved: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Reserved: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Reserved: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1138b2d8d189c54b0da99c29527073068ce6bdf23df4f59fdd7594587486afda");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1388951793;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086475791; // 2084524493

        if (fTestNet)
        {
            block.nTime    = 1388951793;
            block.nNonce   = 387075444;  // 385270584
        }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Reserved: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ReservedMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ReservedMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ReservedMiner(CWallet *pwallet)
{
    printf("ReservedMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("reserved-miner");
    ...
    try { loop {
        ...
        printf("Running ReservedMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ReservedMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ReservedMiner, pwallet));
}
